...
#### Ice Data ####
bridge_cam_ice <- read_csv('../data_station_meta/observed_ice_edge_bridgecam_updated072425.csv')%>%
...
ctd <- read_csv('../processed_data/final_qc_CTD_data_interpolatedGrad.csv')%>%
...
bottle <- read_csv('../processed_data/final_qc_bottle_file.csv')%>%
...
diBiomass_above_trap <- read_csv('../processed_data/depth_integrated_biomass_above_traps.csv')%>%
...
diBiomass_by_station <- read_csv('../processed_data/depth_integrated_biomass_full_wc_all_stations.csv')

#### Trap and Flux Data ####
trap_chlFlux <- read_csv('../processed_data/trap_metadata_with_flux.csv')%>%
...
trap_CNFlux <- read_csv('../processed_data/trap_metadata_CNflux.csv')%>%
  mutate(station = Deploy_Station,
...
#### Trap Speed Data ####
trap_speed_metrics <- read_csv('../data_preprocessed/speed_metrics_all_deployments.csv')%>%
  select(-`...1`)%>%
...
## Formerly useful dataframe that is now not useful
# write_csv(allFlux_forExport, '../processed_data/allTrapFluxData.csv')
...
write_csv(flux_grad, '../processed_data/all_trap_data_for_figs_3_4_5_6_7.csv')
...
# 
# 
# #### Three Boxplots of Flux by Deployment Ice (Bridgecam) ####
# 
...
means_sepIceTethered <- allFlux %>%
  mutate(fluxN = ifelse(fluxN <0, NA, fluxN))%>%  # filters bad point)%>%
  left_join(., bridge_cam_ice)%>%
  mutate(iceObs = ifelse(Deployment == 'T005', 'miz', iceObs),
         iceObs = ifelse(Trap_Depth <=10, 'iceTethered', iceObs))%>%
  group_by(iceObs)%>%
  summarize(meanC = mean(fluxC, na.rm = T),
            sdC = sd(fluxC, na.rm = T),
            meanN = mean(fluxN, na.rm = T),
...
means_front <- flux_grad %>%
  mutate(fluxN = ifelse(fluxN <0, NA, fluxN), # filters bad point
         iceObs = ifelse(Deployment == 'T005', 'miz', iceObs),
         iceObs = ifelse(Trap_Depth <=10, 'iceTethered', iceObs),
         inFront = ifelse(LateralDensityGrad >= 0.02, T, F),
         inFront = ifelse(Trap_Depth <= 10, 'iceTethered', inFront))%>% 
  group_by(inFront)%>%
  summarize(meanC = mean(fluxC, na.rm = T),
            sdC = sd(fluxC, na.rm = T),
            meanN = mean(fluxN, na.rm = T),
...
counts<- left_join(allFlux, bridge_cam_ice)%>%
  filter(str_detect(Sample_Number, 'S$')==T)%>%
  mutate(iceObs = ifelse(Trap_Depth <= 5, 'iceTethered', iceObs))%>%
  select(Deployment, iceObs)%>%
  mutate(iceObs = ifelse(Deployment == 'T005', 'miz', iceObs))%>%
  unique()%>%
  group_by(iceObs)%>%
  count()
...
#### Lateral Density Gradient at 15m By Station ####
ctd%>%
  arrange(station, depth)%>%
  filter(depth == 15)%>%
ggplot(., aes(x= station,
                y = LateralDensityGrad,
                color = section))+
  # geom_point()+
  geom_line()+
  theme_classic()
